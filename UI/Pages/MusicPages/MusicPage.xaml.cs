...
namespace TewiMP.UI.Pages.MusicPages
{
    public partial class MusicPage : Page, IMusicPage
    {
        ...
        public string Title
        {
            get => App.Instance.AudioService.MusicData.Title;
        }
        ...
        private void AddEvents()
        {
            ...
            isAddEvents = true;
            UpdateWhenDataLated();
            LogService.Info(nameof(MusicPage), "Added Events.");
        }
...
#endif
            isAddEvents = false;
            LogService.Info(nameof(MusicPage), "Removed Events.");
        }
        ...
        private void ViewChange(MusicPageViewState musicPageViewState)
        {
            ...
            LogService.Log(nameof(MusicPage), $"ViewState 已被设置为 {musicPageViewState}.");
        }
        ...
        {/*
            ...
            if (ViewState == MusicPageViewState.Hidden)
            {
                ...
                LogManager.Log(nameof(MusicPage), $"LrcBaseListView.ItemSource 已被设置为 null.");
            }*/
        }

        private void PlayingList_NowPlayingImageLoading(Uri imageSource, string _)
        {
            /*if (App.Instance.AudioService.MusicData?.AlbumID != MusicData?.AlbumID)
            {
        ...
        private void PlayingList_NowPlayingImageLoaded(Uri imageSource, string _)
        {
            ...
            LogService.Log(nameof(MusicPage), $"图片已被更改.");
        }
        ...
        private void AudioService_SourceChanged(AudioService AudioService)
        {
            ...
            TitleRunText.Text = AudioService.MusicData.Title;
            Title2RunText.Text = AudioService.MusicData.Title2;
            ArtistRunText.Text = AudioService.MusicData.ArtistName;
            AlbumRunText.Text = AudioService.MusicData.Album.Title;
            OtherRunText.Text = AudioService.MusicData.From == MusicFrom.pluginMusicSource ? AudioService.MusicData.GetMusicSourcePlugin().PluginInfo.Name : MusicFrom.localMusic.ToString();
            AudioInfoRunText.Text = AudioService.WaveInfo;

            if (AudioService.MusicData != MusicData)
            {
        ...
        {
            MediaPlayStateViewer1.PlaybackState = AudioService.PlaybackState;
/*
...
#if DEBUG
            Debug_NowPlayingDurationText.Text = $"{AudioService.CurrentTime} - {App.Instance.LyricService.UpdateInterval}ms";
#endif

            isCodeChangedSliderValue = true;
            PlaySlider.Minimum = 0;
            PlaySlider.Maximum = AudioService.TotalTime.Ticks;
            PlaySlider.Value = AudioService.CurrentTime.Ticks;
            isCodeChangedSliderValue = false;
            NowPlayTimeTb.Text =
                $"{AudioService.CurrentTime:mm\\:ss}/{AudioService.TotalTime.ToString(@"mm\:ss")}";
            NowAtherTimeTb.Text =
                (AudioService.TotalTime - AudioService.CurrentTime).ToString(@"mm\:ss");
        }
        ...
        private void LrcBaseListView_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            if (lrcItem != null )
            {
                ...
                // 加1ms，否则会短时间判定到上一句歌词
                App.Instance.AudioService.CurrentTime = lrcItem.LyricTimeSpan + TimeSpan.FromMilliseconds(App.Instance.AudioService.Latency + 1);
                LogService.Log(nameof(MusicPage), "Changed AudioService current time.");
            }
        }
        ...
        private void PlaySlider_ValueChanged(object sender, Microsoft.UI.Xaml.Controls.Primitives.RangeBaseValueChangedEventArgs e)
        {
            ...
            if (!isCodeChangedSliderValue && !isCodeChangedLrcItem && App.Instance.AudioService.FileReader != null)
            {
                App.Instance.AudioService.CurrentTime = TimeSpan.FromTicks((long)PlaySlider.Value);
            }
        }
        ...
        {/*
            foreach (var data in App.Instance.AudioService.MusicData.Artists)
            {
        ...
        private void ArtistFlyout_Opening(object sender, object e)
        {
            ...
            foreach (var data in App.Instance.AudioService.MusicData.Artists)
            {
                ...
                item.Click += (sender, e) =>
                {
                    ListViewPage.SetPageToListViewPage(new() { PageType = PageType.Artist, Param = (sender as MenuFlyoutItem).Tag });
                    App.MainWindowInstance.OpenOrCloseMusicPage();
                };
                ArtistFlyout.Items.Add(item);
            }
        ...
        {
            ListViewPage.SetPageToListViewPage(new() { PageType = PageType.Album, Param = MusicData.Album });
            App.MainWindowInstance.OpenOrCloseMusicPage();
        }
        ...
        {
            LogService.Info(nameof(MusicPage), $"MusicPage theme changed: {pageRoot.ActualTheme}");
            UpdateAccentColor();
        }
