...
namespace TewiMP.UI.Pages.DialogPages
{
    public partial class AudioInfoPage : Page
    {
        ...
        private async void SetFileSourceInfoText()
        {
            ...
            await Task.Run(() =>
            {
                FileInfo fileInfo = new(App.Instance.AudioService.FileReader.FileName);
                createTime = fileInfo.CreationTime.ToString();
                filePath = fileInfo.DirectoryName;
            });

            ((Run)((Hyperlink)((TextBlock)FileInfoSp.Children[2]).Inlines[0]).Inlines[0]).Text = App.Instance.AudioService.FileReader.FileName;
            ...
            ((TextBlock)FileInfoSp.Children[8]).Text = CodeHelper.GetAutoSizeString(App.Instance.AudioService.FileSize, 2);
        }

        private async void SetAudioInfoText()
        {
            if (App.Instance.AudioService.FileReader != null)
            {
                if (App.Instance.AudioService.FileReader.isMidi)
                {
            ...
            if (App.Instance.AudioService.tfile is null)
            {
                await Task.Run(() =>
                {
                    tfile = new ATL.Track(App.Instance.AudioService.FileReader.FileName);
                });
            }
            else
            {
                tfile = App.Instance.AudioService.tfile;
            }
            ...
            if (tfile != null)
            {
                ...
                ((TextBlock)AudioInfoSp.Children[1]).Text = $"{App.Instance.AudioService.FileType}" +
                    ...
                    $"  {tfile.ChannelsArrangement.NbChannels} 声道  {App.Instance.AudioService.FileReader.TotalTime.ToString("hh\\:mm\\:ss\\.ff")}({tfile.Duration}s)";
        ...
        private async void SetOutInfoText()
        {
            if (App.Instance.AudioService.FileReader.isMidi)
            {
                ((TextBlock)OutInfoSp.Children[2]).Text = $"Midi -> {App.Instance.AudioService.MidiOutputDevice.Name}";
            ...
            else
            {
                ...
                string outInfo = $"未知";
                if ((App.Instance.AudioService.WasapiOnly && App.Instance.AudioService.NowOutDevice.DeviceType == OutApi.Wasapi) || App.Instance.AudioService.NowOutDevice.DeviceType == OutApi.Asio)
                {
                    outInfo = $"{App.Instance.AudioService.NowOutDevice.DeviceType} -> {App.Instance.AudioService.NowOutDevice.DeviceName}";
                }
                else
                {
                    outInfo = $"{App.Instance.AudioService.NowOutDevice.DeviceType} -> SRC -> {App.Instance.AudioService.NowOutDevice.DeviceName}";
                }

                string sampleRateText = "未知";
                string channelsText = "未知";

                if (App.Instance.AudioService.NowOutDevice.DeviceType != OutApi.Asio)
                {
                    //var getd = await OutDevice.GetWasapiDeviceFromOtherAPI(App.Instance.AudioService.NowOutDevice);
                    var outputFormat = await OutDevice.GetWasapiDeviceFromOtherAPI(App.Instance.AudioService.NowOutDevice);
                    //var outputFormat = App.Instance.AudioService.NowOutDevice;
                    if (App.Instance.AudioService.WasapiOnly && App.Instance.AudioService.NowOutDevice.DeviceType == OutApi.Wasapi)
                    {
                        if (App.Instance.AudioService.NowOutObj.OutputWaveFormat.SampleRate != App.Instance.AudioService.FileReader.WaveFormat.SampleRate)
                            sampleRateText = $"{App.Instance.AudioService.FileReader.WaveFormat.SampleRate} Hz -> {App.Instance.AudioService.NowOutObj.OutputWaveFormat.SampleRate} Hz（重采样）";
                        else
                            sampleRateText = $"{App.Instance.AudioService.NowOutObj.OutputWaveFormat.SampleRate} Hz";
                    }
                    else
                    {
                        if (outputFormat.SampleRate != App.Instance.AudioService.FileReader.WaveFormat.SampleRate)
                            sampleRateText = $"{App.Instance.AudioService.FileReader.WaveFormat.SampleRate} Hz -> SRC -> {outputFormat.SampleRate} Hz";
                        else
                            sampleRateText = $"{App.Instance.AudioService.NowOutObj.OutputWaveFormat.SampleRate} Hz（SRC）";

                    }

                    if (App.Instance.AudioService.FileReader.WaveFormat.Channels != outputFormat.Channels)
                    {
                        channelsText = $"{App.Instance.AudioService.FileReader.WaveFormat.Channels} 声道 -> {outputFormat.Channels} 声道";
                    }
                    else
                    {
                        channelsText = $"{App.Instance.AudioService.FileReader.WaveFormat.Channels} 声道";
                    }
                    ((TextBlock)OutInfoSp.Children[10]).Text = $"{App.Instance.AudioService.Latency} ms";
                }
                else
                {
                    var asioOut = App.Instance.AudioService.NowOutObj as AsioOut;
                    if (asioOut.OutputWaveFormat.SampleRate != App.Instance.AudioService.FileReader.WaveFormat.SampleRate)
                        sampleRateText = $"{App.Instance.AudioService.FileReader.WaveFormat.SampleRate} Hz -> {asioOut.OutputWaveFormat.SampleRate} Hz（重采样）";
                    else
                        sampleRateText = $"{asioOut.OutputWaveFormat.SampleRate} Hz";

                    if (App.Instance.AudioService.FileReader.WaveFormat.Channels != asioOut.OutputWaveFormat.Channels)
                    {
                        channelsText = $"{App.Instance.AudioService.FileReader.WaveFormat.Channels} 声道 -> {asioOut.OutputWaveFormat.Channels} 声道";
                    }
                    else
                    {
                        channelsText = $"{asioOut.OutputWaveFormat.Channels} 声道";
                    }
                    ((TextBlock)OutInfoSp.Children[10]).Text = $"{(App.Instance.AudioService.NowOutObj as AsioOut).PlaybackLatency} ms";
                }

                ((TextBlock)OutInfoSp.Children[2]).Text = outInfo;
                ((TextBlock)OutInfoSp.Children[4]).Text = string.IsNullOrEmpty(App.Instance.AudioService.FileReader.DecodeName) ? "未知" : App.Instance.AudioService.FileReader.DecodeName;
        ...
        private async void SetCUEInfoText()
        {
            ...
            if (App.Instance.AudioService.MusicData.CUETrackData is null)
            {
            ...
            {
                return new CueSheet(App.Instance.AudioService.MusicData.CUETrackData.Path);
            });
            if (cueSheet is null)
            {
                CUEInfoGrid.Visibility = Visibility.Collapsed;
            ...
            string nowTrackName = $"标题：{App.Instance.AudioService.MusicData.Title}\n艺术家：{App.Instance.AudioService.MusicData.ArtistName}\n" +
                $"索引：{App.Instance.AudioService.MusicData.CUETrackData.Index}\n" +
                $"开始时间：{App.Instance.AudioService.MusicData.CUETrackData.StartDuration.ToString("hh\\:mm\\:ss\\.ff")}\n" +
                $"结束时间：{App.Instance.AudioService.MusicData.CUETrackData.EndDuration.ToString("hh\\:mm\\:ss\\.ff")}\n" +
                $"时长：{App.Instance.AudioService.MusicData.CUETrackData.Duration.ToString("hh\\:mm\\:ss\\.ff")}";
            ...
            ((TextBlock)CUEInfoSp.Children[2]).Text = App.Instance.AudioService.MusicData.CUETrackData.Path;
